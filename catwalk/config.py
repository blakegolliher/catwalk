...
class Config:
    ...
    def validate(self) -> "Config":
        ...
        integer("CATWALK_PORT", self.port, 1, 65_535)
...
def main():
    """Console entrypoint: `catwalk [--host H] [--port P] [--mock] ...`.

    Flags override the environment, then uvicorn serves catwalk.app:app.
    """
    import argparse

    p = argparse.ArgumentParser(description="Catwalk -- interactive VAST Catalog file browser")
    p.add_argument("--host", help="bind address (env CATWALK_HOST)")
    p.add_argument("--port", type=int, help="bind port (env CATWALK_PORT)")
    p.add_argument(
        "--mock",
        action="store_true",
